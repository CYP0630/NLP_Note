...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    c, W, target = centerWordVec, outsideVectors, outsideWordIdx
    c = c.reshape((c.shape[0], 1))

    #Forward Propogation 
    z = np.dot(W, c)
    preds = softmax(z.reshape(-1)).reshape(-1, 1) 
    ...
    gradOutsideVecs = np.dot(delta, c.T) 
